...
function PetDetail() {
  ...
  function pushRouter() {ipcRenderer.send('router', { window: IWindowList.PET_DETAIL_WINDOW, hash: 'chatgpt' })}
  ...
  function onDrop(e: React.DragEvent<HTMLDivElement>) {
    ...
    if (files?.length) {
      ...
      setDragContent(Object.keys(e.dataTransfer?.files!)
  ...
  return (
    <div>
      {platform != null ? <div style={{ backgroundColor: 'red' }}>功能演示, 平台{platform}</div>: <div>process.platform不存在</div> }
      <div>
        ...
        <div>
          ...
          />
          <button className="btn-info btn-xs btn" onClick={clearAllShortKey}>X</button>
          {/* maxLength="20" */}
        </div>
        <div className="divider">divider</div>
        <div>
      ...
      <div>
        ...
        <div>
          <input className="w-16" placeholder="value" value={keyToDelete} onChange={e => setKeyToDelete(e.target.value)} />
