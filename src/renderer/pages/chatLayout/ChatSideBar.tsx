	...
	return (
		<div className="drawer drawer-end w-0 p-0">
			...
			<div className="drawer-side rounded-md">
				<label htmlFor="my-drawer-4" className="drawer-overlay m-2 rounded-md" onClick={() => setHidden(!hidden)} />
				{hidden ? null : (
					<ul className="menu mr-4 mt-12 h-4/5 w-60 rounded-md bg-base-200 p-4 text-base-content">
						<button className="btn btn-outline rounded-btn btn-sm" onClick={captureCurrentPlugin}>
							capture
						</button>
					</ul>
				)}
