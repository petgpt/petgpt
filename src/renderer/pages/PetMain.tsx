...
function useMove() {
	...
	useEffect(() => {
		ipcRenderer.invoke('db-get', { db: DBList.Config_DB, key: 'Main_Window_Width' }).then(res => setMainW(res))
		ipcRenderer.invoke('db-get', { db: DBList.Config_DB, key: 'Main_Window_Height' }).then(res => setMainH(res))
	}, [])
...
function PetMain() {
	...
	return (
		<div onMouseEnter={onMouseEnterHandler} onMouseLeave={onMouseLeaveWindow} className="pet h-fit">
			...
			{toolsVisible ? (
				<div className="pet-actions flex flex-row content-center items-center justify-center">
					<button className="btn btn-xs" onClick={openDetailWindow}>
						details
					</button>
					<button className="btn btn-xs" onClick={openChatWindow}>
						chat
					</button>
					<button className="btn btn-xs" onClick={openSettingWindow}>
						setting
					</button>
				</div>
			) : null}
