...
import { app as APP, dialog, ipcMain, IpcMainEvent, IpcMainInvokeEvent } from 'electron'
...
export default {
	listen(pluginLoader: PluginLoader, ctx: PetExpose) {
		...
		ipcMain.handle('plugin.getAllPluginName', async (event: IpcMainInvokeEvent, pluginName: string) => {
			return await getPluginList()
		})

		ipcMain.handle('plugin.getConfig', async (event: IpcMainInvokeEvent, pluginName: string) => {
			let plugin = await pluginLoader.getPlugin(pluginName)
			if (!plugin || !plugin.config) return
			return plugin.config(ctx)
		})

		ipcMain.handle('plugin.getSlotMenu', async (event: IpcMainInvokeEvent, args) => {
			let pluginSlotMenuList = []
			let allPluginsNameList = pluginLoader.getAllPluginsNameList()
			for (const name of allPluginsNameList) {
				let plugin: IPetPluginInterface | undefined = await pluginLoader.getPlugin(name)
				if (!plugin || !plugin.slotMenu) continue
		...
		pluginsNameList.forEach(name => {
			...
			ipcMain.on(
				...
				(event: IpcMainEvent, args: { pluginName: string; input: any; reload: boolean }) => {
		...
		ipcMain.on('enablePlugin', async (event: IpcMainEvent, args: { name: string; enabled: boolean }) => {
			...
			const plugin: IPetPluginInterface | undefined = await pluginLoader.getPlugin(`petgpt-plugin-${args.name}`)
			if (!plugin) return
			if (args.enabled) {
				plugin.register()
		...
		const getPluginList = async () => {
			...
			for (const i in pluginList) {
				const plugin: IPetPluginInterface | undefined = await pluginLoader.getPlugin(pluginList[i])
				const pluginPath = path.join(APP.getPath('userData'), `/node_modules/${pluginList[i]}`)
				let pluginPKGText = new TextFileSync(path.join(pluginPath, 'package.json')).read()
				if (!pluginPKGText) {
